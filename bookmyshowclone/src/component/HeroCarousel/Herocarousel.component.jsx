...
const Herocarousel = () => {
        ...
        <HeroSlider {...setting}>
            {
                image.map((image,index) => (
                    <div className='w-full h-56 md:h-80 py-3'key={index}>
                         <img src={`https://images.tmdb.org/t/p/original${image.backdrop_path}`} alt=''className='w-full h-full rounded-md object-cover' />
    ...
    <div className='hidden lg:block'> 
     <HeroSlider {...settingLg} >
     {
          image.map((image ,index)=> (
            <div className="w-full h-56 md:h-80 py-3" key={index}>
              <img src={`https://images.tmdb.org/t/p/original${image.backdrop_path}`} alt="heroBanner" className="w-full h-full rounded-md object-cover" />
            </div>
          ))
        }
        </HeroSlider>
